...
Key env vars: `OPENAI_API_KEY`, `HOSTED_PARAKEET_API_URL` / `MODULATE_API_KEY`, `DEEPGRAM_API_KEY`, `GEMINI_API_KEY` / `ANTHROPIC_API_KEY`, `ENCRYPTION_SECRET` (tests), and `REDIS_DB_HOST`. Firebase Admin uses `SERVICE_ACCOUNT_JSON` / `GOOGLE_APPLICATION_CREDENTIALS`; never commit them. `SERVICE_ACCOUNT_JSON` pins Firestore, GCS, and the customer project. Self-hosted deployments use `OPENAI_BASE_URL` + `SELF_HOSTED_LLM_MODEL` and may store speech profiles under `SPEECH_PROFILE_LOCAL_STORAGE_DIR`. Dev desktop uses `FIREBASE_AUTH_CREDENTIALS_PATH` only for token verification, leaving Google clients on Cloud Run ADC when JSON SA is absent.
